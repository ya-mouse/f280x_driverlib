//#############################################################knowledge interrupt to recieve more interrupts from PIE group 3################
//
//! \file   f280x_common/source/pie.c
//!
...
#include "f280x_common/include/pie.h"
#include "f280x_common/include/pie_init.h"
...
void PIE_enableAdcInt(PIE_Handle pieHandle)
{
    ...
    // set the value
    pie->PIEIER_PIEIFR[0].IER |= PIE_IERx_INTx6_BITS;
...
{
    PIE_Obj *pie = (PIE_Obj *)pieHandle;
    uint16_t setValue = (1 << capNumber);

    // set the value
    pie->PIEIER_PIEIFR[3].IER |= setValue;
                           ...
                           const intVec_t vector)
{
    ...
    intPointer = (intVec_t *)&(pie->SEQ1INT);
