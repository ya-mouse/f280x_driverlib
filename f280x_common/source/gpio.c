//#############################################################################
//
//! \file   f280x_common/source/gpio.c
//!
...
#include "f280x_common/include/gpio.h"
...
void GPIO_setQualification(GPIO_Handle gpioHandle, const GPIO_Number_e gpioNumber, const GPIO_Qual_e qualification)
{
    ...
    {
        uint8_t lShift = gpioNumber << 1;
        uint32_t clearBits = (uint32_t)GPIO_GPQSEL_CONFIG_BITS << lShift;
        uint32_t setBits = (uint32_t)qualification << lShift;

        // clear the bits
        gpio->GPAQSEL1 &= (~clearBits);

        // set the bits
        gpio->GPAQSEL1 |= setBits;
    }
    else if(gpioNumber < GPIO_Number_32)
    {
        uint8_t lShift = (gpioNumber - GPIO_Number_16) << 1;
...
void GPIO_setQualificationPeriod(GPIO_Handle gpioHandle, const GPIO_Number_e gpioNumber, const uint8_t period)
{
    ...
    if(gpioNumber < GPIO_Number_8)
    {
    	gpio->GPACTRL &= ~255;
        gpio->GPACTRL |= (uint32_t)period;
    }
    else if(gpioNumber < GPIO_Number_16)
    {
    	gpio->GPACTRL &= ~(255 << 8);
        gpio->GPACTRL |= (uint32_t)period << 8;
    }
    else if(gpioNumber < GPIO_Number_24)
    {
    	uint32_t clearBits = (uint32_t)255;
    	gpio->GPACTRL &= ~(clearBits << 16);
        gpio->GPACTRL |= (uint32_t)period << 16;
    }
    else if(gpioNumber < GPIO_Number_31)
    {
    	uint32_t clearBits = (uint32_t)255;
    	gpio->GPACTRL &= ~(clearBits << 24);
        gpio->GPACTRL |= (uint32_t)period << 24;
    }
    else if(gpioNumber <= GPIO_Number_34)
    {
       	gpio->GPBCTRL &= ~255;
