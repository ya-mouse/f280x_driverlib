...
//#############################################################################
//
//! \file   f280x_common/include/spi.h
//!
...
inline uint16_t SPI_write(SPI_Handle spiHandle,const uint16_t data)
{
    SPI_Obj *spi = (SPI_Obj *)spiHandle;
...
//! \param[in] spiHandle  The serial peripheral interface (SPI) object handle
//! \param[in] data       The data value
inline uint16_t SPI_write8(SPI_Handle spiHandle,const uint16_t data)
{
    SPI_Obj *spi = (SPI_Obj *)spiHandle;
    ...
    while(!(spi->SPIFFRX & SPI_SPIFFRX_FIFO_ST_BITS)) { };

    return (spi->SPIRXBUF & 0xff);
} // end of SPI_write() function
